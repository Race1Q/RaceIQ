...
const RaceProfileCard: React.FC<RaceProfileCardProps> = ({ race }) => {
  ...
  return (
    <Flex
      ...
      transform={isHovered 
        ? `perspective(1000px) rotateX(${tiltX}deg) rotateY(${tiltY}deg) translateZ(20px)` 
    ...
    >
      ...
      >
        ...
        <Box 
          ...
          top={{ base: 'sm', md: 'lg' }} 
          right={{ base: 'sm', md: 'lg' }}
          transform={isHovered ? `translateZ(25px) scale(1.1)` : "translateZ(0px) scale(1)"}
          transition="transform 0.3s ease"
          borderRadius="sm"
          overflow="hidden"
