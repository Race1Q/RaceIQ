// src/constructor-standings/constructor-standings-ingest.service.ts
...
interface ApiConstructorStanding {
  position: string; // comes back as string
  points: string;
  wins: string;
...
@Injectable()
export class ConstructorStandingsIngestService {
  private readonly logger = new Logger(ConstructorStandingsIngestService.name);
  ...
  private async fetchConstructorStandings(season: number, offset = 0, limit = 30) {
    const apiUrl = `${this.apiBaseUrl}/${season}/constructorstandings.json?limit=${limit}&offset=${offset}`;
    try {
      ...
      if (standingsList.length === 0) {
        this.logger.warn(`No constructor standings found for season ${season}, offset ${offset}`);
        return { standings: [], total: 0 };
      }

      const standings = standingsList[0].ConstructorStandings || [];
      const total = parseInt(response.data.MRData.total) || standings.length;
